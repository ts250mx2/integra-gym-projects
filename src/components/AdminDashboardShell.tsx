...
export default function AdminDashboardShell({ session, children }: Props) {
    ...
    return (
        <div className={theme === 'light' ? 'light-theme' : ''} style={{ minHeight: '100vh', background: 'var(--background)', color: 'var(--foreground)' }}>
            <DashboardHeader
                gymName={session.gymName || 'Admin'}
                userName={session.userName}
                branchName={session.branchName || 'Central'}
                position={session.position || 'Administrador'}
