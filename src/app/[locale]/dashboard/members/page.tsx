...
export default function MembersPage() {
    ...
    if (!mounted) return <div style={{ padding: '1rem' }} className="neon-text">{ct('loading')}</div>;
    ...
    const handleAdd = () => {
        ...
        setCurrentMember({
            ...
            Municipio: branchObj?.Localidad || '', // Mapped from Localidad
            Pais: branchObj?.Pais || '',
            CodigoPostal: '',
            Telefono: '',
            CorreoElectronico: '',
            TarjetaRFID: '',
            Genero: 0, // Default to 0 (Select...)
            ContactoEmergencia: '',
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        if (!currentMember.Genero || currentMember.Genero === 0) {
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        ...
                        <tbody>
                            {filteredMembers.length === 0 ? (
                                <tr>
                                    <td colSpan={6} style={{ padding: '2rem', textAlign: 'center', opacity: 0.5 }}>{t('noData')}</td>
                                </tr>
                            ) : (
                                filteredMembers.map(member => (
                                    <tr key={member.IdSocio} style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                                        <td style={{ padding: '1rem' }}>{member.IdSocio}</td>
                                        ...
                                        <td style={{ padding: '1rem', textAlign: 'center' }}>
                                            <div style={{ display: 'flex', justifyContent: 'center', gap: '0.5rem' }}>
                                                ...
                                                <button onClick={() => handleDelete(member.IdSocio)} className="btn-icon" style={{ color: '#ff4444' }} title={ct('delete')}>
                                                    <Trash2 size={16} />
                                                </button>
                                            </div>
                                        </td>
                                    ...
                                    <div style={{ display: 'flex', gap: '1rem' }}>
                                        <div style={{ flex: 1 }}>
                                            ...
                                            <div className="input-field" style={{ opacity: 0.6, background: 'rgba(255,255,255,0.05)' }}>
                                                {currentMember.IdSocio ? `${getBranchClave(Number(currentMember.IdSucursal))}${currentMember.IdSocio}` : '...'}
                                            </div>
                                        </div>
                                        <div style={{ flex: 2 }}>
                                            ...
                                            <select
                                                ...
                                                onChange={e => {
                                                    ...
                                                    setCurrentMember({
                                                        ...
                                                        Municipio: branch?.Localidad || currentMember.Municipio,
